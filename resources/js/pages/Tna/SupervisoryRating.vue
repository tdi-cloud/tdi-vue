...
const donePct = computed(() => (requiredIds.length ? Math.round((doneCount.value / requiredIds.length) * 100) : 100));
...
async function runFasdSearch() {
    ...
    fasdOpen.value = true;
    try {
        const res = await fetch(route('tna.fasd.search') + '?q=' + encodeURIComponent(fasdQuery.value), { headers: { Accept: 'application/json' } });
        ...
        <div class="relative z-10 mx-auto max-w-5xl space-y-6">
            ...
            <div class="rounded-2xl bg-white p-8 shadow-xl">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span class="rounded-full bg-blue-600 px-4 py-1.5 text-xs font-bold tracking-wide text-white"> TNA {{ assessment.period }} </span>
                </div>
                ...
                <div class="mt-6 rounded-xl border border-sky-300 bg-sky-50/60 p-5">
                    ...
                    <div class="mt-2 grid grid-cols-1 gap-x-8 gap-y-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input
                                v-model="form.subordinate_position"
                                type="text"
                                class="mt-1.5 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                            />
                        </div>
                                    ...
                                    <div class="space-y-3">
                                        <div v-for="scale in SCALES" :key="scale.key">
                                            ...
                                            <div class="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-2">
                                                ...
                                                >
                                                    <input
                                                        v-model="answers[el.competency_id][scale.key]"
                                                        type="radio"
                                                        :value="opt"
            ...
            <div class="rounded-2xl bg-white p-8 shadow-xl">
                ...
                <p class="mb-4 text-sm text-gray-500">
                    Select the FASD of your region who will be the "Noted by" signatory on the TNA Result. This is required before you can submit.
                </p>
                ...
                <!-- Selected chip -->
                <div v-if="selectedFasd" class="mt-4 flex items-center justify-between rounded-xl border border-green-200 bg-green-50 px-4 py-3">
                    <div>
                    ...
                    <button type="button" class="text-xs font-semibold text-red-600 hover:underline" @click="clearFasd">Change</button>
                </div>
            ...
            <div class="rounded-2xl bg-white p-8 shadow-xl">
                <label class="block text-sm font-medium leading-relaxed text-gray-700">
                    ...
                    electronic signature has the same legal force and effect as a manual signature. This step is optional — you may leave it blank and
                    add your signature later.
                </label>
                ...
                <div class="mt-6 flex flex-wrap items-center gap-4">
                    ...
                    <span v-if="donePct < 100" class="text-xs text-gray-500"> {{ requiredIds.length - doneCount }} more element(s) to answer. </span>
                    <span v-else-if="!form.fasd_empcode" class="text-xs text-gray-500"> Please select the FASD signatory above. </span>
                </div>
            </div>
