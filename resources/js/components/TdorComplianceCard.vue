...
const modalTitle = computed(() => {
    ...
    return modalRegion.value === 'ALL'
        ? `TDOR — ${label} (All Regions)`
        : `TDOR — ${label} · ${modalRegion.value}`;
});
</script>
...
<template>
    <div class="rounded-2xl border bg-card shadow-sm flex flex-col gap-5 p-5">
        ...
        <div class="flex items-center gap-2">
            <div class="h-8 w-8 rounded-lg bg-violet-100 dark:bg-violet-950/50 flex items-center justify-center shrink-0">
        ...
        <div v-if="loading" class="flex justify-center py-12">
            <div class="h-8 w-8 rounded-full border-4 border-violet-500 border-t-transparent animate-spin" />
        </div>
        ...
        <template v-else-if="data">
            ...
            <div class="grid grid-cols-3 gap-3">
                ...
                <button
                    class="rounded-xl p-3 flex flex-col gap-0.5 text-left hover:brightness-95 transition-all cursor-pointer border border-violet-200 dark:border-violet-800/40 bg-violet-50 dark:bg-violet-950/20"
                    @click="fetchList('submitted', 'ALL')"
                >
                    <p class="text-[10px] font-semibold uppercase tracking-wide flex items-center gap-1 text-violet-700 dark:text-violet-400">
                ...
                <button
                    class="rounded-xl p-3 flex flex-col gap-0.5 text-left hover:brightness-95 transition-all cursor-pointer border border-orange-200 dark:border-orange-800/40 bg-orange-50 dark:bg-orange-950/20"
                    @click="fetchList('not_submitted', 'ALL')"
                >
                    <p class="text-[10px] font-semibold uppercase tracking-wide flex items-center gap-1 text-orange-600 dark:text-orange-400">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y">
                            ...
                            >
                                ...
                                <td class="px-4 py-2.5">
                                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300">
                                        {{ emp.region }}
                                    </span>
                                </td>
