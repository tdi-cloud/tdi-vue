...
async function handleFileChange(e: Event) {
    ...
    try {
        await axios.post(route('foreign-programs.nhrdc-signed-copy.upload', [props.programId, props.nhrdcMemberId]), data);
        emit('uploaded');
    } catch (err: any) {
...
async function destroy() {
    ...
    processing.value = true;
    try {
        await axios.delete(route('foreign-programs.nhrdc-signed-copy.destroy', [props.programId, props.nhrdcMemberId]));
        emit('deleted');
    } finally {
        processing.value = false;
    }
...
<template>
    <div class="flex flex-wrap items-center gap-2">
        <a
            v-if="hasFile"
            :href="route('foreign-programs.nhrdc-signed-copy.download', [programId, nhrdcMemberId])"
            target="_blank"
            rel="noopener"
