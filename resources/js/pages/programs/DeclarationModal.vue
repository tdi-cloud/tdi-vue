...
import { AlertTriangle, CheckCircle2, Download, FileBadge2, LoaderCircle, Search } from 'lucide-vue-next';
...
const completedCount = computed(() => (props.batch?.participants ?? []).filter((p: any) => p.attendance === 'Complete').length);
...
const generate = () => {
    ...
    const url = route('batches.declaration', props.batch.id) + `?signatory_empcode=${encodeURIComponent(selected.value.empcode)}`;
...
<template>
    <Dialog :open="open" @update:open="emit('update:open', $event)">
        <DialogContent class="max-w-md !rounded-2xl">
            <DialogHeader>
                <DialogTitle>
                    <span class="flex items-center gap-2"> <FileBadge2 class="h-5 w-5 text-blue-600" /> Generate Declaration of Completers </span>
                </DialogTitle>
            ...
            <!-- Wala pang Complete -->
            <div v-if="completedCount === 0" class="flex flex-col items-center gap-2 py-6 text-center">
                ...
                <p class="max-w-xs text-xs text-slate-400">
                    No participants have "Complete" attendance for this batch yet. Set at least one participant's attendance in the Participants tab
            ...
            <div v-else class="flex flex-col gap-3">
                ...
                <div class="grid gap-1">
                    ...
                    <div v-if="!selected" class="relative">
                        ...
                        <Input v-model="query" class="h-8 pl-7 text-xs" placeholder="Search employee to sign (name or empcode)..." />
                        <LoaderCircle
                            v-if="searching"
                            class="absolute right-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 animate-spin text-muted-foreground"
                        />

                        <div
                            v-if="showDropdown && results.length"
                            class="absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border bg-popover shadow-md"
                        >
                            <button
                            ...
                            >
                                <span>
                                    <span class="block font-semibold">{{ emp.name }}</span>
                        ...
                        <div
                            ...
                            class="absolute z-50 mt-1 w-full rounded-lg border bg-popover px-3 py-2 text-xs text-muted-foreground shadow-md"
                        >
                            No employees found.
                        </div>
                    </div>
                    ...
                    <div v-else class="flex items-center justify-between rounded-lg border px-3 py-2">
                        ...
                        <Badge variant="secondary" class="cursor-pointer text-[10px]" @click="selected = null"> Change </Badge>
                    </div>
                </div>

                <div class="mt-1 flex justify-end gap-2 border-t pt-2">
                    <Button variant="outline" size="sm" @click="emit('update:open', false)">Cancel</Button>
                    <Button size="sm" class="bg-blue-600 hover:bg-blue-700 dark:text-white" :disabled="!selected || generating" @click="generate">
                        <LoaderCircle v-if="generating" class="mr-1 h-3 w-3 animate-spin" />
