...
const CRITICALITY_GUIDE = [
    [1, 'Slightly important'],
    [2, 'Moderately important'],
    [3, 'Highly important'],
];
const COMPETENCE_GUIDE = [
    [0, 'Not competent'],
...
const allElements = [...props.coreUnits.flatMap((u) => u.elements), ...props.electiveUnits.flatMap((u) => u.elements)];
...
const corePct = computed(() => (requiredIds.length ? Math.round((coreDone.value / requiredIds.length) * 100) : 100));
...
async function runSupSearch() {
    ...
    supOpen.value = true;
    try {
        const res = await fetch(route('tna.supervisor.search') + '?q=' + encodeURIComponent(supQuery.value), {
...
const canSubmit = computed(() => !!form.supervisor_empcode && corePct.value === 100);
...
async function runChgSearch() {
    ...
    try {
        const res = await fetch(route('tna.supervisor.search') + '?q=' + encodeURIComponent(chgQuery.value), {
            headers: { Accept: 'application/json' },
        });
...
<template>
    ...
    <div class="tna-page relative min-h-screen px-4 py-8">
        ...
        <div class="relative z-10 mx-auto max-w-5xl space-y-6">
            <Link :href="route('home')" class="inline-flex items-center gap-1.5 text-sm font-medium text-white/80 transition hover:text-white">
            ...
            <div v-if="flashSuccess" class="rounded-xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-medium text-green-800 shadow">
                {{ flashSuccess }}
            </div>

            <!-- Already submitted state -->
            <div v-if="alreadySubmitted" class="rounded-2xl bg-white p-8 text-center shadow-xl">
                ...
                <p class="mt-2 text-sm text-gray-600">
                    For TNA period <b>{{ period }}</b
                    >. Submitted on {{ alreadySubmitted.submitted_at }}. Please wait for the supervisory rating from your selected supervisor.
                </p>
                <div class="mt-5 flex flex-wrap items-center justify-center gap-3">
                    <Link
                        v-if="alreadySubmitted.id && alreadySubmitted.reviewed"
                        :href="route('tna.result.show', alreadySubmitted.id)"
                ...
                <div v-if="!alreadySubmitted.reviewed" class="mx-auto mt-6 max-w-md border-t border-gray-100 pt-5 text-left">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" class="text-xs font-semibold text-blue-600 hover:underline" @click="chgOpen = !chgOpen">
            ...
            <template v-else>
                ...
                <div class="rounded-2xl bg-white p-8 shadow-xl">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            <h1 class="text-2xl font-bold tracking-tight text-gray-900">ASSESSMENT OF CURRENT COMPETENCIES</h1>
                            <p class="mt-1 text-sm text-gray-500">Self Rating</p>
                        </div>
                        <span class="rounded-full bg-blue-600 px-4 py-1.5 text-xs font-bold tracking-wide text-white"> TNA {{ period }} </span>
                    </div>
                ...
                <div class="rounded-2xl bg-white p-8 shadow-xl">
                    ...
                    <p class="mb-4 text-sm text-gray-500">
                        Your selected supervisor will provide the supervisory rating after your self-rating. This is required before you can submit.
                    </p>
                    ...
                    >
                        ...
                        <button type="button" class="text-xs font-semibold text-red-600 hover:underline" @click="clearSupervisor">Change</button>
                    </div>
                ...
                <div class="rounded-2xl bg-white p-6 shadow-xl">
                    <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
                        ...
                        <p class="mt-1 text-sm leading-relaxed text-gray-700">
                            Below are the units of competencies required in the performance of your job. Using the scale below, rate the competency
                            units according to its
                            <b>CRITICALITY</b> to your job, your level of <b>COMPETENCY</b>, and <b>FREQUENCY</b> of utilization. Aside from the list,
                            choose any of the elective competencies that you need in your work, if applicable. Please answer carefully as this
                            assessment will determine your Professional Development Plan.
                        </p>
                    </div>
                ...
                <div v-if="electiveUnits.length" class="space-y-8">
                    <div class="rounded-2xl bg-white px-6 py-4 shadow-xl">
                        ...
                        <p class="mt-1 text-xs text-gray-500">Please rate all of the following competencies.</p>
                    </div>
                ...
                <div class="rounded-2xl bg-white p-8 shadow-xl">
                    <label class="block text-sm font-medium leading-relaxed text-gray-700">
                        You may sign below using your mouse, finger, or an uploaded photo. By signing, you understand and agree that this form of
                        electronic signature has the same legal force and effect as a manual signature. This step is optional — you may leave it blank
                        and add your signature later.
                    </label>
            ...
            <!-- FOOTER -->
            <footer class="flex items-center justify-center gap-2 pb-2 pt-4">
                <img src="https://upload.wikimedia.org/wikipedia/commons/e/ef/TESDA_Seal.svg" alt="TESDA Seal" class="h-6 w-6" />
