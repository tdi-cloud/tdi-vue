...
import {
    X, History, Search, ChevronLeft, ChevronRight, Loader2,
    UserRound, Building2, FileText, CheckCircle2, ExternalLink,
    ...
    <Transition name="backdrop" appear>
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="emit('close')">
            <Transition name="pop" appear>
                <div class="bg-background rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">

                    <!-- Header -->
                    <div class="shrink-0 bg-gradient-to-r from-indigo-600 via-violet-600 to-blue-600 border-b px-6 py-4 flex items-center gap-3 text-white">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-white/75 mt-0.5">All nomination form submissions, latest to oldest</p>
                        </div>
                    ...
                    <!-- Filters -->
                    <div class="shrink-0 p-4 border-b flex flex-col sm:flex-row gap-2">
                        <div class="relative flex-1">
                            ...
                            <input
                                ...
                                class="w-full border rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-background"
                            />
                        </div>
                        <select v-model="filterSponsor" class="border rounded-lg px-2 py-2 text-xs bg-background shrink-0">
                            <option value="">All Sponsors</option>
                            <option v-for="s in sponsorOptions" :key="s" :value="s">{{ s }}</option>
                        </select>
                        <select v-model="filterStatus" class="border rounded-lg px-2 py-2 text-xs bg-background shrink-0">
                    ...
                    <div class="flex-1 overflow-y-auto p-4">
                        <div v-if="loading" class="flex items-center justify-center gap-2 text-xs text-muted-foreground py-10">
                            <Loader2 class="h-4 w-4 animate-spin" /> Loading...
                        </div>

                        <div v-else-if="nominees && nominees.data.length > 0" class="flex flex-col gap-2">
                            <div v-for="n in nominees.data" :key="n.id" class="rounded-xl border overflow-hidden">
                                <div
                                    role="button"
                                    tabindex="0"
                                    class="w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-muted/40 transition-colors cursor-pointer"
                                ...
                                >
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            <p class="font-bold text-sm leading-tight">{{ n.name }}</p>
                                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0" :class="statusColors[n.status]">
                                                {{ n.status_label }}
                                            </span>
                                        </div>
                                        ...
                                        <p v-if="n.program_title" class="text-[11px] mt-0.5 truncate">
                                            <Link
                                                v-if="n.program_id"
                                    ...
                                    <div class="flex flex-col items-end gap-1 shrink-0">
                                        <span class="text-[10px] text-muted-foreground whitespace-nowrap">{{ formatDateTime(n.submitted_at) }}</span>
                                        <span
                                            v-if="n.requirements_total > 0"
                                            class="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full"
                                            :class="n.requirements_submitted >= n.requirements_total
                                ...
                                <div v-if="expandedId === n.id" class="border-t bg-muted/20 px-4 py-3">
                                    <p class="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-2">Submitted Requirements</p>
                                    <div v-if="n.submissions.length > 0" class="flex flex-col gap-1.5">
                                        <a
                                            ...
                                            target="_blank"
                                            class="flex items-center gap-2 rounded-lg border bg-background px-3 py-2 text-xs hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors"
                                        >
                                            <FileText class="h-3.5 w-3.5 text-blue-500 shrink-0" />
                                            <span class="flex-1 min-w-0 truncate font-medium">{{ sub.question ?? 'Requirement' }}</span>
                                            <ExternalLink class="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                                        </a>
                                    </div>
                                    <p v-else class="text-xs text-muted-foreground italic">No requirement documents submitted yet.</p>
                                </div>
                            </div>
                        ...
                        <p v-else class="text-xs text-muted-foreground text-center py-10">No nomination submissions found.</p>
                    </div>
                </div>
            </Transition>
        </div>
