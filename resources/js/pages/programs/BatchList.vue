...
/* ---------- ✅ BAGO: View participants ---------- */
const showParticipants = ref(false);
const viewingBatchId = ref<number | null>(null);

// Galing sa props ang batch para laging fresh ang data
// kahit mag-reload ang Inertia pagkatapos mag-add/remove
...
const computeHours = () => {
    if (form.time_start && form.time_end && form.days) {
        ...
        if (perDay > 5) perDay -= 1; // less 1hr lunch break
        if (perDay > 0) {
...
<template>
    <div class="flex flex-col gap-3 mt-5">
        ...
        <!-- ✅ BAGO: Participant finder — hanapin kung saang batch naka-enroll -->
        <ParticipantSearch
            v-if="batches.length"
            :batches="batches"
        ...
        <div v-if="!batches.length" class="flex flex-col items-center justify-center rounded-2xl border border-dashed py-14 px-6 text-center gap-3">
            <!-- Illustration: calendar / schedule -->
        ...
        <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-2">
            ...
            >
                ...
                <div class="mt-3 space-y-1.5 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <!-- ✅ participant count galing sa loaded participants -->
                    <p class="flex items-center gap-1.5">
                ...
                <div class="mt-3 flex justify-end gap-1 border-t pt-2 px-2">
                    ...
                    >
                        <Eye class="h-3 w-3" /> View Participants
                    </Button>
                    ...
                    >
                        <FileBadge2 class="h-3 w-3" /> Declaration
                    </Button>




                    <Button variant="ghost" size="sm" class="h-7 px-2 text-xs" @click="openEdit(batch)">
                        <Pencil class="h-3 w-3" /> Edit
                    </Button>
                    <Button variant="ghost" size="sm" class="h-7 px-2 text-xs text-red-500 hover:text-red-600" @click="destroy(batch)">
