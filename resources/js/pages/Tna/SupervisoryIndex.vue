...
async function redoRating(id) {
    if (
        !(await confirmDialog('Clear your rating and start over? Your ratings for this subordinate will be deleted.', {
            confirmText: 'Clear Rating',
        }))
    )
        return;
        ...
        <div class="relative z-10 mx-auto max-w-4xl space-y-6">
            ...
            <div class="rounded-2xl bg-white p-8 shadow-xl">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    ...
                    <span v-if="pendingCount" class="font-semibold text-blue-600"> {{ pendingCount }} pending review. </span>
                </p>
            </div>
            ...
            <div v-else class="space-y-3">
                <div v-for="a in assessments" :key="a.id" class="rounded-2xl bg-white p-5 shadow-xl">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                <p class="text-base font-semibold text-gray-900">{{ a.name }}</p>
                                <span v-if="a.reviewed" class="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-700">
                                    Rated
                                </span>
                                <span v-else class="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700"> Pending </span>
                            </div>
                            <p class="mt-0.5 text-sm text-gray-600">{{ a.position }}</p>
                            <p class="mt-0.5 text-xs text-gray-400">
                                {{ a.office }}<span v-if="a.division"> · {{ a.division }}</span> · TNA {{ a.period }} · Submitted {{ a.submitted_at }}
                            </p>
                        </div>
                    ...
                    <div v-if="a.reviewed" class="mt-4 border-t border-gray-100 pt-4">
                        <TnaScanUpload :assessment-id="a.id" type="supervisory" label="Supervisory Rating" :has-file="a.supervisor_scan_uploaded" />
                    </div>
                </div>
            </div>
        </div>
